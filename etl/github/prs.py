...
DEFAULT_REPOS = [r.strip() for r in os.environ.get("GITHUB_REPOS", "").split(",") if r.strip()]
...
def ensure_collection():
    ...
    for field in ("repo", "author", "state", "type", "date"):
        requests.put(
...
def main():
    ...
    if org:
        ...
        repos = DEFAULT_REPOS
        if not repos:
            print("No repos configured. Set GITHUB_REPOS in .env, or pass --org/--repos.")
            return
